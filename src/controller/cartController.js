...
const deleteCart = async (req, res) => {
    try {
        ...
        let cartdeleted = await cartModel.findOne({ userId:userId })
        if( cartdeleted.items.length == 0 ) { return res.status(404).send({status: false, message: "No cart Found" }) }
        
        let deletedCart = await cartModel.findOneAndUpdate({ userId: userId }, { items: [], totalItems: 0, totalPrice: 0 }, { new: true }).select({__v:0})
